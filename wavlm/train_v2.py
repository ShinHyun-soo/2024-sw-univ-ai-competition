...
AUDIO_MODEL_NAME = 'abhishtagatya/wavlm-base-960h-asv19-deepfake'
...
class MyLitModel(pl.LightningModule):
    def __init__(self, audio_model_name, num_labels, n_layers=1, projector=True, classifier=True, dropout=0.07,
        ...
        self.audio_model = WavLMForSequenceClassification.from_pretrained(audio_model_name, config=self.config)
    ...
    def _do_reinit(self, n_layers=0, projector=True, classifier=True):
        ...
        for n in range(n_layers):
            self.audio_model.wavlm.encoder.layers[-(n + 1)].apply(self._init_weight_and_bias)
...
if __name__ == '__main__':
    ...
    submission_df.to_csv(os.path.join(SUBMISSION_DIR, 'wavlm_deepfake_fold_2_pure_datset_val_interval_1_epoch0.csv'), index=False)